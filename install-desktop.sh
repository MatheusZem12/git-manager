#!/usr/bin/env bash
#
# Instala (ou remove) o Git Manager como um aplicativo do desktop: cria o
...
COMMENT="Gerenciador local de repositórios Git"
...
Name=$APP_NAME
Comment=$COMMENT
Exec=env -u ELECTRON_RUN_AS_NODE GTK_USE_PORTAL=0 "$ELECTRON" --ozone-platform=x11 --xdg-portal-required-version=5 "$SRC"
Path=$SRC
Icon=$APP_ID
Terminal=false
