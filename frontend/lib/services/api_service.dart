...
class ApiService {
  ...
  Future<GitProject> addProject(String name, String path, {String notes = '', String group = ''}) async {
    final data = await _post('/projects', {
      'name': name,
      'path': path,
      'notes': notes,
      'group': group,
  ...
  Future<GitProject> updateProject(GitProject project) async {
    final data = await _put('/projects', project.toJson());
    return GitProject.fromJson(data);
  }
